...
If you're connecting to Azure Database for PostgreSQL Single Server, the connection string looks like:
...
The command generates a config file called `dab-config.json` and looks like this:
...
that updates the configuration file and the `book` entity looks like the following code:
