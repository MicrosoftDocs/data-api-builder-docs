...
Data API builder 2.0 focuses on MCP and AI integration, simpler authentication defaults, better configuration automation, stronger observability, and more flexible REST and OpenAPI behavior.

## Authentication and authorization

### `Unauthenticated` is now the default authentication provider

DAB 2.0 introduces `Unauthenticated` as a new authentication provider and makes it the default for new configurations. When this provider is active, all requests run as `anonymous`. DAB does not inspect or validate any JWT. Authentication is expected to be handled upstream, for example by Azure API Management or an application gateway.
...
If `authenticated` is not explicitly configured for an entity, it inherits from `anonymous`. If a named role is not configured, it inherits from `authenticated`, or from `anonymous` if `authenticated` is also absent.

#### Why?

With this, you can define permissions once on `anonymous` and every broader role gets the same access automatically, no duplication required.
...
Now, when you're unsure what a role can actually do after all inheritance rules are applied, you can get the answer in one command without reasoning through the config manually.
...
With this, you can build APIs where the SQL database sees and enforces the real user identity, which is required for row-level security policies and compliance auditing. This feature is available only for `mssql` data sources and requires Entra ID authentication upstream.
...
> [!NOTE]
> This applies only when OBO authentication is active. Standard deployments are unaffected.
...
With this, you can wire up an entire database schema as a DAB API without writing a single entity block by hand. Define your patterns once and DAB does the rest.
...
New configurations generated by `dab init` now include a default OpenTelemetry section pre-wired to standard OTEL environment variables.
...
If the environment variables are not set, DAB starts normally and skips OTLP exporter setup. Unresolved `@env(...)` values are tolerated at startup.
...
With this, you can expose only the entities that make sense for AI tools, disable MCP on sensitive tables, and enable custom tool generation for stored procedures—all without changing your REST or GraphQL configuration.
...
The OpenAPI document now shows only the HTTP methods and fields that are actually accessible based on permissions. A new role-specific path `/openapi/{role}` lets you see exactly what a given role can do.

#### Why?

With this, your OpenAPI document is a reliable contract. Clients and API consumers see only what they're allowed to call, not the full internal surface.
...
DAB uses longest-prefix matching for routing. Path traversal patterns, backslashes, and percent-encoded path separators in config values are blocked by validation.
...
With this, you can insert a new row with a server-generated key using the same upsert semantics you're used to—no need to pre-generate or supply the key yourself.
...
For composite keys, any non-auto-generated portions must still be supplied. Stored procedures are unaffected. OpenAPI documents these keyless operations on the base entity path automatically.
...
With this, you can confirm at a glance what role DAB is using to evaluate a request, which is especially handy when debugging auth configuration or upstream header forwarding.
...
This works automatically when OpenTelemetry is configured. Combined with the default telemetry section now generated by `dab init`, new apps have out-of-box observability for all three endpoint types.
...
With this, you have a working starting point for Aspire-based architectures where DAB is one component in a larger orchestrated system.
