...
3. MSSQL database configured to accept Microsoft Entra ID tokens
...
DAB 2.0 introduces `Unauthenticated` as a new authentication provider and makes it the default for new configurations. When this provider is active, all requests run as `anonymous`. DAB doesn't inspect or validate any JSON Web Token (JWT). Even if another service in front of DAB authenticates the caller, DAB still evaluates permissions only as `anonymous`.

### Why?

Now, when you want DAB to stay `anonymous`-only, you don't need to configure a JWT provider just to get started. Running `dab init` produces a working config without any extra auth options.
...
Now, you can group related entities under a shared path prefix, making your REST API feel more naturally organized without any router configuration. This approach can be helpful for multitenant scenarios where a tenant ID segment can segment endpoints with the same name, like `/api/shopping-cart/item` and `/api/invoice/items`, which previously required unique entity names.
...
- [Advanced REST paths with subdirectories](../concept/rest/overview.md#advanced-rest-paths-with-subdirectories)
...
- [Permission-aware OpenAPI](../concept/rest/openapi.md#permission-aware-openapi)
- [Role-specific OpenAPI paths](../concept/rest/openapi.md#role-specific-openapi-paths)
...
- [HTTP response compression](../concept/rest/overview.md#http-response-compression)
...
- [Keyless PUT and PATCH](../concept/rest/overview.md#keyless-put-and-patch-for-autogenerated-primary-keys)
